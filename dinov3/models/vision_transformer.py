...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]):
        ...
        for blk_index, blk in enumerate(self.blocks):
            ...
            x = blk(x, rope_sincos)
            if self.layers_to_extract_from is not None:
                if blk_index in self.layers_to_extract_from:
                    for idx, (x_item, masks) in enumerate(zip(x, masks_list)):
                        if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
                            if self.untie_global_and_local_cls_norm and self.training and idx == 1:
                                # Assume second entry of list corresponds to local crops.
                                # We only ever apply this during training.
                                x_norm_cls_reg = self.local_cls_norm(x_item[:, : self.n_storage_tokens + 1])
                            elif self.untie_cls_and_patch_norms:
                                x_norm_cls_reg = self.cls_norm(x_item[:, : self.n_storage_tokens + 1])
                            else:
                        ...
                        output[idx]["x_norm_clstoken"].append(x_norm_cls_reg[:, 0])
                        output[idx]["x_storage_tokens"].append(x_norm_cls_reg[:, 1:])
                        output[idx]["x_norm_patchtokens"].append(x_norm_patch)
                        output[idx]["x_prenorm"].append(x_item)
                        output[idx]["masks"].append(masks)
            else:
                if blk_index == len(self.blocks) - 1:
                    for idx, (x_item, masks) in enumerate(zip(x, masks_list)):
                        if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
                            if self.untie_global_and_local_cls_norm and self.training and idx == 1:
                                # Assume second entry of list corresponds to local crops.
