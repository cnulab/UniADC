...
def dinov3_vitl16_dinotxt_tet1280d20h24l(
    *,
    ...
    bpe_path_or_url: str = "bpe_simple_vocab_16e6.txt.gz",
...
class DinoV3TextEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(DinoV3TextEncoder, self).__init__()
        ...
        self.tokenizer = get_tokenizer(bpe_path_or_url=bpe_path_or_url)


    def encode_text(self, prompts, normalize: bool = False) -> torch.Tensor:
        toks = self.tokenizer.tokenize(prompts).to(self.device)
        features = self.text_model(toks)
